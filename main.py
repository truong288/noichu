...
async def announce_next_turn(update, context):
    ...
    mention = f"<a href='tg://user?id={next_id}'>{chat.username or chat.first_name}</a>"
...
async def announce_winner(update, context):
    ...
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=f"🏆 {mention} Vô Địch Nối CHỮ! (Tổng số chiến thắng: {stats[name]})",
        parse_mode="HTML"
    )
    reset_game_state()
...
async def turn_timer(context):
    ...
    try:
        ...
        if len(players) == 1:
            ...
            if current_player_index >= len(players):
                current_player_index = 0
            await announce_next_turn(None, context)
